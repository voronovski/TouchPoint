...
struct MissingDatesRegression {
    ...
    static func main() {
        ...
        expect(store.peopleMissingDates().map(\.id) == [friend.id, client.id, family.id, colleague.id],
        ...
        for occasion in Occasion.allCases {
            ...
            expect(!store.peopleMissingDates().contains { $0.id == friend.id },
        ...
        expect(lastPerson.peopleMissingDates().isEmpty,
        ...
        expect(store.peopleMissingDates().map(\.id) == [friend.id, family.id, colleague.id],
               "Deleted people must disappear from the list")
        print("PASS: all date types, all relationships, ordering, profile edits, final date removal, empty state, deletion")
    }
}
